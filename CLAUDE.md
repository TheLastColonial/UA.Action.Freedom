...
**Two write-once records, both enforced the same way.** `Convoy.TruckListPublishedAt` and `Box.ValidatedAt`/`ValidatedByPersonId` are stamped by a dedicated `POST` transition and are *absent from the corresponding `UPDATE` statement and request body*, so no ordinary edit can set, clear or forge them. Each transition's SQL is conditional (`WHERE ... AND TruckListPublishedAt IS NULL` / `AND ValidatedAt IS NULL`), so the database settles a race rather than a read-then-write in C#. Both then freeze their aggregate: a published truck list will not take or release a vehicle, and a validated box will not take or release an item or change receiver — in each case because a manifest or a border document would otherwise be describing something that is no longer true. Follow this shape for the manifest lifecycle.
