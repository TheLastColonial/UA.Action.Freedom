...
copy to delete. See `docs/domain/key-concepts.md` § Volunteer erasure.
...
1. **Do dispatchers and loaders need the system to work offline or on poor connections?** Loading happens in a
   ...
   > No assume that we have good signal at all sites.
2. **Who is accountable if a manifest is wrong at the border?** That determines how strictly the system must
   ...
   > Once a GMR is created, no edits can be created to modify the manifest
3. **Does the ELO need to be generated by Freedom, or is it produced elsewhere and attached?** The current design
   ...
   > ELO needs to be generated by Freedom, I assume that there is a goverment API that creates this. However the API needs to be identified.
   Possibly On the EU/import side:
    - ENS (Entry Summary Declaration) is what EU member states require for goods entering
    - This is handled by the haulier/carrier, usually before goods arrive at the border
    - Each EU member state has its own customs procedures; there's no single "EU API"
    - France specifically uses NCTS (New Computerised Transit System) for transiting goods, and DELTA (Déclaration Électronique du Transit) for electronic declarations
...
4. **How many convoys run concurrently?** The design assumes one convoy cycle at a time. Overlapping convoys
   ...
   > There is only 1 convoy a month, it could become 2 a month or weekly but not concurrently.
