...
test('the shell renders the dashboard at the index route', async () => {
  const screen = await renderWithProviders(null, { routes, route: '/', roles: ['Administrator'] });
...
test('a section the role cannot read is absent from the nav', async () => {
  const screen = await renderWithProviders(null, { routes, route: '/', roles: ['GroundOfficer'] });
